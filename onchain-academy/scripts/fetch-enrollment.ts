...
async function main() {
  ...
  if (!enrollment) {
    console.log(
      `No enrollment found for ${learner.toBase58()} in course "${courseId}"`
    );
    return;
  }
  ...
  console.log(
    "Enrolled:         ",
    new Date(enrollment.enrolledAt.toNumber() * 1000).toISOString()
  );
  console.log(
    "Completed:        ",
    enrollment.completedAt
      ? new Date(enrollment.completedAt.toNumber() * 1000).toISOString()
      : "in progress"
  );
  console.log(
    "Credential Asset: ",
    enrollment.credentialAsset?.toBase58() || "none"
  );
  console.log(`Progress:          ${completed}/${total} lessons`);
  console.log(
    "Lessons:          ",
    renderBitmap(enrollment.lessonFlags as BN[], total)
  );
}
